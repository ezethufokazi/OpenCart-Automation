...
public class CartPage extends BasePage {
    ...
    public boolean verifyCorrectProduct(String productName) {
        try {
            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
            wait.until(ExpectedConditions.visibilityOf(lnkProductName));
            return lnkProductName.getText().contains(productName);
        } catch(Exception e) {
            return false;
        }
    }
    ...
    // clears quantity field and enters new value
    public void updateQuantity(String quantity) {
        txtQuantity.clear();
        txtQuantity.sendKeys(quantity);
    }

    // clicks update button using JS to avoid overlay issues
